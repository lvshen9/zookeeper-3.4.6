...
import org.apache.zookeeper.acl.SourceAddressControl;
            ...
            while((!shutdown) && (numRetries < 3)){
                try {
                    ...
                    while (!shutdown) {
                		...
                		/** 原地址访问控制 */
                		if (SourceAddressControl.isEnable()) {

                			// 格式: "/127.0.0.1:7572"
                			String address = client.getRemoteSocketAddress().toString();
                			String strIp = address.substring(1, address.lastIndexOf(":"));
                			
                			if (!SourceAddressControl.isPermit((strIp))) {
                				LOG.warn("deny illegal access, remote-address:" + strIp + ", local port:" + client.getLocalPort());
                				client.close();
                				continue;
                			}
                		}
                        ...
                        setSockOpts(client);
                        LOG.info("Received connection request " + client.getRemoteSocketAddress() + ", local port:" + client.getLocalPort());
